...
def pairwise_interpolate_predictions(
    ...
    for idx, point_from_j in tqdm(enumerate(reprojected_j)):
        ...
        point_from_j_nns = np.concatenate((uv_i[point_nn_indexes],image_i.reshape(-1,1)[point_nn_indexes]),axis=1)
        ...
        distances_to_nearest = sum(np.sum((point_from_j-point_from_j_nns)**2,axis=1)**0.5)
        interp_mask[idx] = distances_to_nearest < distance_interpolation_threshold
